...
export default function JournalPage() {
  ...
  const weeklyStats: WeeklyStats[] = React.useMemo(() => {
    ...
    return Object.entries(weeks)
      .map(([week, data]) => ({ week: Number(week), pnl: data.pnl, days: data.days.size }))
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2"><BarChart className="h-5 w-5" />Account Stats</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <MetricWidget title="Win Percentage" value={overallMetrics.winRate.toFixed(1)} unit="%" description="Percentage of trades that were profitable." />
                <MetricWidget title="Profit Factor" value={overallMetrics.profitFactor === Infinity ? "∞" : overallMetrics.profitFactor.toFixed(2)} description="Gross profit divided by gross loss." />
                <MetricWidget title="Trade Expectancy" value={overallMetrics.expectancy.toLocaleString("en-US", { style: "currency", currency: "USD" })} description="Average P/L you can expect per trade." />
              </CardContent>
            </Card>
          </div>
        </div>
        ...
        {selectedDayStats && (
          <Card>
            <CardHeader><CardTitle>Trades for {format(selectedDay!, "PPP")}</CardTitle></CardHeader>
            <CardContent>
              <div className="space-y-2">
                {selectedDayStats.tradeList.map((trade, idx) => (<TradeListItem key={idx} trade={trade} />))}
              </div>
            </CardContent>
          </Card>
        )}
